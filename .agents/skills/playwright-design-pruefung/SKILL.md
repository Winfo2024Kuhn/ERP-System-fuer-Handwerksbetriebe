...
| `react-pc-frontend` | `pc-uebergang` | 1536 × 960 | Übergangsgröße an responsiven Breakpoints. Läuft für jede PC-Spec mit. |
...
Alle drei Desktop-Projekte laufen für jede PC-Spec automatisch mit. Eine Spec, die nur in
...
npx playwright test e2e/meine.spec.ts          # nur eine Spec, alle drei Desktop-Größen
...
**Zeiterfassung.** Die vorhandene `react-zeiterfassung/playwright.config.ts`
verwenden: Projekt `handy` mit `devices['iPhone 15']` und Chromium, `E2E_PORT`
(Standard 5175), `baseURL` `https://127.0.0.1:<port>/zeiterfassung/`.
`ignoreHTTPSErrors: true` ist für das lokale HTTPS bereits gesetzt; der Vite-Server
startet mit `--strictPort`. `@playwright/test` und das Script `test:e2e` sind bereits
in `react-zeiterfassung/package.json` vorhanden. Vorhandene Specs unter
`react-zeiterfassung/e2e/` ergänzen, keine zweite Konfiguration anlegen.

```bash
# Aus react-zeiterfassung/
E2E_PORT=5176 npm run test:e2e -- --project=handy
```
...
- **Design-Reviewer** (`loese-problem-design-review`, eigener Agent, nur bei
  Frontend-Änderungen): fährt `npm run test:e2e` komplett (alle Specs, alle drei Desktop-
  Größen bzw. `handy` für die Zeiterfassung) in einem **eigenen Worktree** auf dem gemergten Stand, schaut jeden
...
PC-Specs importieren `test` und `expect` aus `e2e/hilfen/test.ts` (wie im Beispiel
oben). Die gemeinsame Fixture ruft `blockiereFremdeNetzwerkzugriffe` aus
`e2e/hilfen/api.ts` bereits für den BrowserContext auf; damit sind auch Popups
abgedeckt. Nicht zusätzlich dieselbe Sperre je Spec kopieren. Zur Kontrolle einmal
`page.on('request', …)` mitschreiben und zusichern, dass keine fremde **Antwort**
ankommt.
