...
## Abschnitt 10 — Code-Review (Code-Reviewer)

Zeit: 2026-09-06T21:55:00Z
Branch: feature/layout-14-zoll @ b722f20e (Diff 677654b0..HEAD, 29 Dateien, 814 Zeilen)
Commit(s): keine — Review ist read-only, alle Mutationsproben restlos zurückgenommen (`git status` nach jedem Schritt leer)
Status: fertig
Ampel: 🟡
...
- **Gates (aus `react-pc-frontend/`, synchron):**
  - `npm run lint`: exit 0, 0 Fehler, **1 Warnung** (`src/pages/BelegeKasseEditor.tsx:1204`, react-hooks/exhaustive-deps) — identisch zur Baseline.
  - `npm run typecheck:e2e`: sauber. **Wirksamkeit geprüft** (Mutationsprobe): eine absichtlich falsch typisierte Zeile in `e2e/hilfen/design.ts` wird gemeldet (`TS2322: Type 'string' is not assignable to type 'number'`), Datei danach zurückgesetzt. Der Lauf deckt wirklich `e2e/` ab — `tsconfig.json` referenziert nur `tsconfig.app.json` (`include: ["src"]`) und `tsconfig.node.json`, `e2e/` war vorher komplett ungetypt.
  - `npm run build`: exit 0, nur die vorbestehende Chunk-Größen-Warnung. Build-Output verworfen (`git checkout -- src/main/resources/static/index.html`, neue Asset-Dateien gelöscht).
  - `npm run test`: 88 Dateien, **1082 Tests, 1074 grün / 8 rot**. Alle acht sind `Test timed out in 5000ms`, alle in Dateien, die dieser Abschnitt nicht anfasst (ArtikelEditor, LieferantenEditor, document-editor/index, BeitraegeTab, BeitragAssistent). Isolierter Nachlauf dieser fünf Dateien: **77/77 grün**. Nach der Abnahmeregel der Baseline damit grün, kein Assertion-Fehler.
- **Änderungsumfang:** 28 Dateien unter `react-pc-frontend/`, 1 unter `docs/`. Nichts außerhalb, kein Backend, kein `./mvnw`.

- **Mutationsprobe 1 — ist der scharfe Standard scharf?** Ja. Eigene Probe-Spec (danach gelöscht): `designPruefung()` mit **Standardoptionen** auf einer Miniseite mit unmarkierter `truncate`-Kürzung wird **rot**; dieselbe Seite mit `data-kuerzung-erlaubt` bleibt grün. Die Arbeitsteilung trägt: `keinTextLaeuftUeber` überspringt Ellipsis-Elemente, `keinTextGekuerzt` fängt sie mit der schärferen Schwelle (+1 statt +2) wieder ein. Die Ausnahme ist nicht zu weit geraten — mit **einer** Ausnahme, siehe Hinweis 2.
- **Mutationsprobe 2 — findet `keineUeberschneidungen` noch echte Überlappungen?** Ja. Zwei überlappende Knöpfe **innerhalb** eines sichtbaren Scrollbereichs: rot. Sticky-Leiste gegen fest positionierten Toast: rot. Zwei fest positionierte Knöpfe unter einem `overflow:hidden`-Vorfahren: rot (der `position: fixed`-Abbruch in `sichtbaresRechteck` greift korrekt). Das Clipping schneidet also nicht pauschal alles weg.
- **Fundstelle 2 aus Task 10b geklärt** (im Log als „reproduziert nicht mehr, Ursache nicht zweifelsfrei" offen gelassen): Ursache ist eindeutig Block 1. `LieferantDokumentModal.tsx` hängt die `BearbeitenLeiste` in eine `shrink-0`-Kopfzeile **über** einen `overflow-hidden`-Inhaltsbereich. Ein nach oben weggescrolltes Formularfeld behielt sein volles Rechteck hinter dieser Leiste — der Browser klippt es, die alte Prüfung nicht. Die 18 roten Fälle bei 1536x960 waren echte Fehlalarme, das Clipping ist die richtige Reparatur.
- **Netz-Riegel:** alle 17 Specs importieren `test`/`expect` aus `./hilfen/test` (geprüft, keine Ausnahme), kein Spec legt einen eigenen `browser.newContext()` an, keine manuellen `blockiereFremdeNetzwerkzugriffe()`-Aufrufe mehr übrig. `globalSetup` (`aufwaermen.ts`) ist mit abgedeckt — es baut Browser/Page von Hand und bekommt den Riegel jetzt vor der ersten Navigation.
- **`role="dialog"`:** 21 Dateien binden `ui/dialog` ein. Kein Unit-Test und keine Spec bricht: die vorhandenen `getByRole('dialog', { name: ... })`-Selektoren filtern über den Namen, und die `ui/dialog`-Dialoge tragen keinen (`role="dialog"` ohne `aria-label`/`aria-labelledby` hat keinen zugänglichen Namen). Attribute stehen **vor** `{...props}`, ein Aufrufer kann sie also weiterhin überschreiben.
- **`pc-uebergang` für alle Specs:** trägt. 1536px ist genau Tailwinds `2xl`-Breakpoint — die interessanteste Breite der ganzen App, und 960px eine dritte, vorher ungeprüfte Höhe. `erwarteteKartenspalten()` bildet die Tailwind-v3-Standardbreakpoints korrekt ab (`tailwind.config.js` hat keinen eigenen `screens`-Block, alle vier Kartenraster nutzen dieselbe Klassenkette). Der Sprung auf 384 Tests ist verdient, nicht bloß Laufzeit.

Bedenken / Abweichungen vom Plan:

- **Hinweis 1 (der wichtigste): abgelaufener Ausstieg in `e2e/menueleiste-layout.spec.ts`.** Vier `designPruefung()`-Aufrufe rufen weiterhin `strengePruefungen: testInfo.project.name === 'pc-monitor'` — schalten die scharfen Prüfungen bei **pc-14zoll (1440)** und **pc-uebergang (1536)** also ab. Die Begründung dafür steht im Dateikopf und ist mit Abschnitt 10 hinfällig geworden: „keinTextLaeuftUeber kennt noch keine Ausnahme für die gewollte `max-w-[10rem] truncate`-Kürzung … das ist die im Plan (Task 10) angekündigte Nacharbeit an design.ts". Genau die hat Block 1 geliefert. **Nachgemessen:** mit `strengePruefungen: true` in allen vier Aufrufen läuft die Spec **15/15 grün über alle drei Größen** (`E2E_PORT=5199`, 1,5 Min); Datei danach zurückgesetzt. Damit läuft `keinTextLaeuftUeber` ausgerechnet auf der Menüleiste — der Komponente, die auf jeder Seite steht — bei der Titel-Größe des Vorhabens nicht mit. Fix: die vier `strengePruefungen:`-Zeilen streichen und den Dateikopf-Absatz nachziehen. Eine Zeile Arbeit, belegt grün.
- **Hinweis 2: blinder Fleck bei `line-clamp` + Breiten-Überlauf.** Zweig (b) der neuen Ausnahme (`istReineTextKuerzung`) nimmt auch `-webkit-line-clamp`-Elemente aus — aber `keinTextGekuerzt` prüft bei line-clamp nur die **Höhe** (`scrollHeight > clientHeight`), nie die Breite. Gemessen an einer Miniseite: ein `line-clamp`-Kasten mit 80px Breite und 462px Textbreite (382px still abgeschnitten) wird von **keiner** der drei Prüfungen gemeldet. Vor Abschnitt 10 fing `keinTextLaeuftUeber` das noch. Im Code betrifft das 29 der 34 `line-clamp`-Stellen, die keinen Marker tragen (z.B. `LieferantenEditor.tsx:866`, `<span className="line-clamp-2">{adresse}</span>` — ein bindestrichloser Straßenname verschwindet dort lautlos nach rechts). Fix ist eine Zeile: in `keinTextGekuerzt` die line-clamp-Bedingung um `|| el.scrollWidth > el.clientWidth + 1` erweitern.
- **Hinweis 3: `sichtbaresRechteck()` klippt absolut positionierte Elemente zu scharf.** Ein `overflow:hidden`-Vorfahre ohne eigene `position` klippt ein `position: absolute`-Kind **nicht** (sein Containing Block liegt weiter oben) — die Funktion schneidet trotzdem. Nachgemessen: ein Menüpanel, das laut `getBoundingClientRect()` voll sichtbar bei (200,200) steht und einen anderen Knopf komplett verdeckt, wird **nicht** gemeldet. Die `position: fixed`-Abbruchbedingung deckt den Fall nicht ab. Betrifft das Dropdown-in-Karte-Muster; im heutigen Code entschärft, weil die Klipper meist selbst `relative` sind. Fix: für `position: absolute`-Elemente nur Vorfahren schneiden, die selbst positioniert sind.
- **Hinweis 4: die Sticky-Regel nimmt mehr aus als nötig.** `if (a.sticky !== b.sticky && !a.fest && !b.fest) continue;` überspringt jede Sticky-gegen-Inhalt-Überlappung, auch die, die **ohne Scrollen** besteht — also den echten Layoutfehler „die Leiste liegt dauerhaft über einem Knopf". Nachgemessen: nicht gemeldet. Für die heutigen sieben Sticky-Stellen (RibbonNav, Tabellenköpfe, Modal-Kopf/Fuß) unkritisch, aber die Prüfung kann eine dauerhaft verdeckte Primäraktion nicht mehr finden, und `primaerAktion.toBeInViewport()` prüft Sichtbarkeit, nicht Verdeckung. Genauer wäre: nur ausnehmen, wenn das sticky Element gerade tatsächlich versetzt ist (aktuelle Position ≠ statische Position).
- **Hinweis 5: `role="dialog"` hat eine Nebenwirkung außerhalb der E2E-Prüfung.** `src/components/ui/toast.tsx` entscheidet per `document.querySelector('[role="dialog"]')`, ob der Toast-Container nach oben links wandert. Mit dem neuen Attribut löst das jetzt für **21 weitere Komponenten** aus, die `ui/dialog` einbinden — sichtbare Verhaltensänderung in Produktion, im Kommentar an der Änderung nicht erwähnt. Inhaltlich ist sie richtig (der Toast soll die Modal-Fußleiste nicht verdecken), sie gehört nur benannt.
- **Hinweis 6: `aria-modal="true"` ohne zugänglichen Namen und ohne Fokus-Fang.** Der Fokus-Fang ist bewusst und nachvollziehbar ausgelassen. `aria-modal` sagt der Vorlesesoftware allerdings, dass alles außerhalb nicht existiert, während der Tab-Fokus weiter dorthin wandern kann — und ohne `aria-label`/`aria-labelledby` wird der Dialog namenlos angesagt. Kein Regress (die anderen Modale im Projekt machen es genauso), aber eine offene Baustelle, die zu den fehlenden Reiter-Rollen passt.
- **Hinweis 7 (klein):** `sichtbaresRechteck()` ruft `el.getBoundingClientRect()` viermal hintereinander für dasselbe Element — einmal reicht. Bei rund 100 interaktiven Elementen pro Seite mal 384 Tests ist das umsonst bezahltes Reflow.
