...
export function Select({ options, value, onChange, placeholder = 'Bitte wählen...', className, disabled, id, name, required, error, 'aria-label': ariaLabel, 'aria-describedby': describedBy }: SelectProps) {
    ...
    useLayoutEffect(() => {
        ...
        return () => { window.removeEventListener('resize', update); window.removeEventListener('scroll', update, true); document.removeEventListener('pointerdown', outside); };
    }, [isOpen, disabled]);
    useEffect(() => {
    ...
    return <div className={cn('relative w-full', className)}>
        <button ref={triggerRef} id={inputId} type="button" role="combobox" disabled={disabled}
            ...
            className="flex min-h-12 w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-slate-50"
        ...
        {isOpen && !disabled && ReactDOM.createPortal(<div ref={dropdownRef} id={listId} role="listbox" aria-label={ariaLabel ?? 'Auswahl'}
            className="overflow-auto rounded-lg border border-slate-200 bg-white p-1 text-slate-950 shadow-2xl"
            style={{ position: 'fixed', ...position, zIndex: 10020 }} onMouseDown={event => event.preventDefault()}>
            {options.length === 0 ? <div className="px-2 py-2 text-center text-sm text-slate-500">Keine Optionen</div> : options.map((option, index) =>
                <div key={option.value} id={`${listId}-${index}`} role="option" aria-selected={value === option.value} aria-disabled={option.disabled}
                    className={cn('relative flex min-h-12 items-center rounded-lg py-3 pl-3 pr-8 text-base', option.disabled ? 'cursor-not-allowed text-slate-400' : 'cursor-pointer hover:bg-rose-50 hover:text-rose-900', activeIndex === index && !option.disabled && 'bg-rose-50 text-rose-900', value === option.value && 'font-medium')}
