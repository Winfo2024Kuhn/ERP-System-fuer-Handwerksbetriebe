...
export default function AngebotNotizenPage() {
    const { angebotId } = useParams<{ angebotId: string }>()
    ...
    async function loadNotizen() {
        ...
        try {
            ...
            const res = await fetch(`/api/angebote/${angebotId}/notizen?token=${token}`)
            if (res.ok) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const url = editingNotiz
                ? `/api/angebote/${angebotId}/notizen/${editingNotiz.id}`
                : `/api/angebote/${angebotId}/notizen`
    ...
    const handleDelete = async (notizId: number) => {
        ...
        try {
            const token = localStorage.getItem('zeiterfassung_token')
            const res = await fetch(`/api/angebote/${angebotId}/notizen/${notizId}?token=${token}`, {
                method: 'DELETE'
            })
            if (res.ok) {
                loadNotizen()
    ...
    const handleImageUpload = async (notizId: number, file: File) => {
        ...
        try {
            ...
            const res = await fetch(`/api/angebote/${angebotId}/notizen/${notizId}/bilder?token=${token}`, {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
    ...
    const handleDeleteImage = async (notizId: number, bildId: number) => {
        ...
        try {
            const token = localStorage.getItem('zeiterfassung_token')
            const res = await fetch(`/api/angebote/${angebotId}/notizen/${notizId}/bilder/${bildId}?token=${token}`, {
                method: 'DELETE'
            })
            if (res.ok) {
                loadNotizen()
    ...
    const handleOpenEditModal = (notiz: Notiz) => {
                                ...
                                <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {notiz.nurFuerErsteller && (
                                                    <span className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-amber-700">
                                    ...
                                    {notiz.canEdit && (
                                        <div className="flex flex-col gap-1 ml-2">
                                            <button
                                                onClick={() => handleOpenEditModal(notiz)}
            ...
            {showModal && (
                ...
                >
                    ...
                    >
                        ...
                        <textarea
                            className="w-full h-32 p-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500"
