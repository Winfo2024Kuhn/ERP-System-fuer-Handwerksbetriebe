...
class MonatsabschlussUebersichtServiceTest {
    ...
    @Test void summenUndAuswahlUmfassenAlleSeitenUndHistorieOhneLiveOderAudit() {
        ...
        when(repo.personen(isNull(),isNull(),any())).thenReturn(people);when(repo.salden(anyList(),eq(2025*12+12),eq(2025*12+12))).thenReturn(rows);
    ...
    @Test void offeneInvalidierteCachesWerdenBerechnetStatusDanachGefiltert() {
        when(repo.personen(eq(1L),eq(2L),any())).thenReturn(List.of(person(1)));
    ...
    @Test void sechsMonateUeberJahresgrenzeEinSetAbruf() {
        when(repo.personen(any(),any(),any())).thenReturn(List.of(person(1)));var rows=new ArrayList<MonatsSaldo>();for(int i=8;i<=12;i++)rows.add(stand(1,2024,i,true));rows.add(stand(1,2025,1,true));when(repo.salden(anyList(),eq(2024*12+8),eq(2025*12+1))).thenReturn(rows);
