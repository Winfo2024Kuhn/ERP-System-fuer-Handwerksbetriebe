...
class MonatsabschlussUebersichtSecurityTest {
    ...
    @Test void anonym401() throws Exception { for(String path:List.of("/uebersicht","/vergleich")) mvc.perform(get(B+path).param("jahr","2025").param("monat","1").header("X-Mitarbeiter-Id","1")).andExpect(status().isUnauthorized());mvc.perform(post(B+"/sammelabschluss").with(csrf()).contentType("application/json").content("{\"auswahl\":[]}")).andExpect(status().isUnauthorized()); }
    @Test @WithMockUser void alleEndpunkteVerlangenRecht() throws Exception { for(String path:List.of("/uebersicht","/vergleich"))mvc.perform(get(B+path).param("jahr","2025").param("monat","1")).andExpect(status().isForbidden());mvc.perform(post(B+"/sammelabschluss").with(csrf()).contentType("application/json").content("{\"auswahl\":[]}")).andExpect(status().isForbidden());verifyNoInteractions(repo,saldo,tm); }
