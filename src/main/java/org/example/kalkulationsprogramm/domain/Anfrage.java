...
public class Anfrage {
    ...
    @OneToMany(mappedBy = "anfrage", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<AnfrageDokument> dokumente = new ArrayList<>();

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "anfrage_kunden_emails", joinColumns = @JoinColumn(name = "anfrage_id"))
    @Column(name = "email")
    ...
    @OneToMany(mappedBy = "anfrage", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.LAZY)
    private List<AnfrageNotiz> notizen = new ArrayList<>();
