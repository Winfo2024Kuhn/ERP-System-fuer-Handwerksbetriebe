...
public class EinkaufLieferung {
 ...
 @JdbcTypeCode(SqlTypes.VARCHAR)@Column(name="idempotenz_key",nullable=false,length=36)private UUID idempotenzKey;@Column(name="akteur_id",nullable=false)private Long akteurId;@Column(name="angelegt_am",nullable=false)private Instant angelegtAm=Instant.now();@OneToMany(mappedBy="lieferung",cascade=CascadeType.ALL)private List<LieferungPosition> positionen=new ArrayList<>();
