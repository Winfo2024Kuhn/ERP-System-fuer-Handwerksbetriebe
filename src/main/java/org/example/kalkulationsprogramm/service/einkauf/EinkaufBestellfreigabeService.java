...
@Service public class EinkaufBestellfreigabeService {
 ...
 private final EinkaufBestellungRepository orders;private final BestellungRevisionRepository revisions;private final AngebotVersionRepository offers;private final EinkaufKommunikationVorschauRepository previews;private final EinkaufVorlagenService templates;private final EinkaufPdfService pdf;private final EinkaufDateiService files;private final EinkaufOutboxService outbox;private final EinkaufVersandWorker worker;private final EinkaufVersandauftragRepository outboxRepo;private final EinkaufMengenService amounts;private final EinkaufBedarfRepository needs;private final LieferantDokumentRepository documents;private final EinkaufAuditService audit;private final ObjectMapper json;private final EinkaufZeugnisService zeugnisse;
 public EinkaufBestellfreigabeService(EinkaufBestellungRepository orders,BestellungRevisionRepository revisions,AngebotVersionRepository offers,EinkaufKommunikationVorschauRepository previews,EinkaufVorlagenService templates,EinkaufPdfService pdf,EinkaufDateiService files,EinkaufOutboxService outbox,EinkaufVersandWorker worker,EinkaufVersandauftragRepository outboxRepo,EinkaufMengenService amounts,EinkaufBedarfRepository needs,LieferantDokumentRepository documents,EinkaufAuditService audit,ObjectMapper json,EinkaufZeugnisService zeugnisse){this.orders=orders;this.revisions=revisions;this.offers=offers;this.previews=previews;this.templates=templates;this.pdf=pdf;this.files=files;this.outbox=outbox;this.worker=worker;this.outboxRepo=outboxRepo;this.amounts=amounts;this.needs=needs;this.documents=documents;this.audit=audit;this.json=json;this.zeugnisse=zeugnisse;}
 ...
 private void bucheAnnahme(EinkaufBestellung order,BestellungRevision revision,UUID key,Long actor,Instant zeit) {
  ...
  zeugnisse.erwarte(revision.getId(),localDate(revision.getSnapshot().get("liefertermin")));
 ...
 private static LocalDate localDate(Object value){if(value==null)return null;if(value instanceof LocalDate date)return date;return LocalDate.parse(value.toString());}
