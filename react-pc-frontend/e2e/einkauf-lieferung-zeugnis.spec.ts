...
interface E2eErwartung {
  ...
  chargeIds: number[]; materialFreigegeben: boolean; zuordnungen?: Array<{ zuordnungId: number; erwartungId: number; chargeId: number; status: string; version: number; materialFreigegeben: boolean }>;
...
test('erfasst die zweite Teillieferung und ordnet ein Zeugnis mehreren Chargen zu, bevor es geprüft wird', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/einkauf/zeugnisse/504/zuordnen', async route => {
    const body = route.request().postDataJSON(); createdPayloads.push(body);
    erwartet = [{ ...expectation(), dateiIds: [504], lieferPositionIds: [91, 92], chargeIds: [101, 102], status: 'ZUGEORDNET', zuordnungen: [
  ...
  await page.route('**/api/einkauf/zeugnisse/601/pruefen', route => { erwartet = erwartet.map(item => ({ ...item, zuordnungen: item.zuordnungen?.map(link => link.zuordnungId === 601 ? { ...link, status: 'GEPRUEFT', version: link.version + 1, materialFreigegeben: true } : link) })); return route.fulfill({ json: { id: 701, erwartungId: 111, ergebnis: 'BESTANDEN', begruendung: 'Dummy-Prüfung', grundlageVersion: '2026', akteurId: 1, geprueftAm: '2026-09-24T12:00:00Z', materialFreigegeben: true } }); });
  await page.route('**/api/einkauf/zeugnisse/602/pruefen', route => { erwartet = erwartet.map(item => ({ ...item, zuordnungen: item.zuordnungen?.map(link => link.zuordnungId === 602 ? { ...link, status: 'GEPRUEFT', version: link.version + 1, materialFreigegeben: false } : link) })); return route.fulfill({ json: { id: 702, erwartungId: 111, ergebnis: 'ABGELEHNT', begruendung: 'Falsche Schmelznummer', grundlageVersion: '2026', akteurId: 1, geprueftAm: '2026-09-24T12:00:00Z', materialFreigegeben: false } }); });
  ...
  await designPruefung(page, testInfo, 'einkauf-lieferdialog', { primaerAktion: page.getByRole('button', { name: 'Lieferung erfassen' }).last() });
  ...
  await designPruefung(page, testInfo, 'einkauf-zeugnispruefung', { primaerAktion: page.getByRole('button', { name: 'Prüfung speichern' }) });
