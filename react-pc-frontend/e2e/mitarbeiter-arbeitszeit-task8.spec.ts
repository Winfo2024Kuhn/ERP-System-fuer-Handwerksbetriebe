...
test('Task 8: Arbeitszeit bewusst einrichten und Vorschau prüfen', async ({ page }, testInfo) => {
    let vorschauBody: Record<string, unknown> | null = null;
    await page.route('**/api/**', (route) => {
        ...
        if (pfad === '/api/zeitverwaltung/zeitkonten/42/vorschau' && methode === 'POST') {
            vorschauBody = route.request().postDataJSON() as Record<string, unknown>;
    ...
    expect(vorschauBody).toMatchObject({ vorlageId: 3, expectedVorlageVersion: 2, arbeitszeit: { montagStunden: 7.5 } });
    await page.getByText('Vorschau', { exact: true }).scrollIntoViewIfNeeded();
