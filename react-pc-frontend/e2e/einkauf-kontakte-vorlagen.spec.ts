...
test('Einkaufsvorlage lädt passende Tokens und lässt die Standardvariante gezielt wechseln', async ({ page }) => {
 ...
 await page.route('**/api/**',async route=>{const url=new URL(route.request().url());const path=url.pathname;const method=route.request().method();if(path==='/api/auth/me')return route.fulfill({json:{id:1,username:'max.mustermann',displayName:'Max Mustermann',active:true,roles:['ADMIN'],admin:true,requiresInitialSetup:false}});if(path==='/api/notifications/summary')return route.fulfill({json:{totalCount:0,categories:[],recentItems:[]}});if(path==='/api/email-textvorlagen/dokumenttypen')return route.fulfill({json:[{value:'EINKAUF_ANFRAGE',label:'Einkauf — Lieferantenanfrage',kategorie:'EINKAUF',kategorieLabel:'Einkauf'}]});if(path==='/api/email-textvorlagen/placeholders')return route.fulfill({json:[{token:'{{ANREDE}}',label:'Anrede'}]});if(path==='/api/email-textvorlagen/placeholders/EINKAUF_ANFRAGE')return route.fulfill({json:[{token:'{{ANREDE}}',label:'Neutrale Anrede',pflicht:true,imBetreffErlaubt:false}]});if(path==='/api/email-textvorlagen')return route.fulfill({json:[{id:8,dokumentTyp:'EINKAUF_ANFRAGE',kategorie:'EINKAUF',name:'Anfrage Variante A',subjectTemplate:'Anfrage {{PA_NUMMER}}',htmlBody:'<p>{{ANREDE}}</p>',aktiv:true,standard:false},{id:9,dokumentTyp:'EINKAUF_ANFRAGE',kategorie:'EINKAUF',name:'Anfrage Variante B',subjectTemplate:'Anfrage {{PA_NUMMER}}',htmlBody:'<p>{{ANREDE}}</p>',aktiv:true,standard:true}]});if(path==='/api/email-textvorlagen/8'&&method==='PUT'){gespeichert=route.request().postDataJSON();return route.fulfill({json:{id:8,...gespeichert,standard:true}});}return route.fulfill({json:[]});});
