...
test('Anfrage prüfen, hashgebunden freigeben und Verlauf nach Neuladen sehen', async ({ page }, testInfo) => {
 let status='AUSSTEHEND'; const freigaben: unknown[]=[]; let templateSubject='Anfrage {{ANFRAGENUMMER}}'; let gesendeterBetreff='';
 ...
 await page.route('**/api/einkauf/anfragen**', async route => {
  ...
  if(url.pathname==='/api/einkauf/anfragen/51/revisionen/1') return route.fulfill({json:{angezeigteRevisionId:1,historisch:true,kopf:{id:51,version:0,paNummer:'PA-00051',aktuelleRevisionId:2,revisionsNummer:1,status:'VERSENDET'},positionen:[],lieferanten:[{id:3,lieferantenname:'Alter Anbieter',status:'VERSENDET',version:0}]}});
  if(url.pathname==='/api/einkauf/anfragen/51/revisionen/2') return route.fulfill({json:{angezeigteRevisionId:2,historisch:false,kopf:{id:51,version:0,paNummer:'PA-00051',aktuelleRevisionId:2,revisionsNummer:2,status,antwortfrist:'2026-10-01',liefertermin:'2026-10-14'},positionen:[{id:9,snapshot:{bezeichnung:'Profil Dummy',basis:{menge:2,einheit:'STUECK'}},herkuenfte:[]}],lieferanten:[{id:4,lieferantenname:'Musterlieferant',status:'AUSSTEHEND',version:0,kontakt:{lieferantId:4,kontaktId:9,lieferantenname:'Musterlieferant',email:'einkauf@example.test',name:'Einkauf',anrede:'Guten Tag',eigeneKundennummer:'00017'}}]}});
  if(url.pathname==='/api/einkauf/anfragen/51/lieferanten/4/senden' && route.request().method()==='POST'){const request=route.request().postDataJSON();freigaben.push(request);gesendeterBetreff=request.vorschauHash==='hash-pa-51'?'Anfrage PA-00051':'';status='VERSENDET';return route.fulfill({json:{beteiligungId:4,status:'ANGENOMMEN'}});}
  if(url.pathname==='/api/einkauf/anfragen/51') return route.fulfill({json:{angezeigteRevisionId:2,historisch:false,kopf:{id:51,version:0,paNummer:'PA-00051',aktuelleRevisionId:2,revisionsNummer:2,status,antwortfrist:'2026-10-01',liefertermin:'2026-10-14'},positionen:[{id:9,snapshot:{bezeichnung:'Profil Dummy',basis:{menge:2,einheit:'STUECK'}},herkuenfte:[]}],lieferanten:[{id:4,lieferantenname:'Musterlieferant',status:'AUSSTEHEND',version:0,kontakt:{lieferantId:4,kontaktId:9,lieferantenname:'Musterlieferant',email:'einkauf@example.test',name:'Einkauf',anrede:'Guten Tag',eigeneKundennummer:'00017'}}]}});
 ...
 await page.route('**/api/email-textvorlagen**', async route=>{const url=new URL(route.request().url());if(url.pathname.endsWith('/einkauf-vorschau'))return route.fulfill({json:{subject:route.request().postDataJSON().subjectTemplate.replace('{{ANFRAGENUMMER}}','PA-2026-00001'),htmlBody:'<p>Guten Tag</p>'}});if(url.pathname.endsWith('/dokumenttypen'))return route.fulfill({json:[{value:'EINKAUF_ANFRAGE',label:'Einkauf — Lieferantenanfrage',kategorie:'EINKAUF',kategorieLabel:'Einkauf'}]});if(url.pathname.includes('/placeholders/'))return route.fulfill({json:[{token:'{{ANFRAGENUMMER}}',label:'Anfragenummer',pflicht:true,imBetreffErlaubt:true},{token:'{{ANREDE}}',label:'Anrede',pflicht:true,imBetreffErlaubt:false}]});if(url.pathname.endsWith('/placeholders'))return route.fulfill({json:[{token:'{{ANFRAGENUMMER}}',label:'Anfragenummer'}]});if(route.request().method()==='PUT'){templateSubject=route.request().postDataJSON().subjectTemplate;return route.fulfill({json:{id:7,dokumentTyp:'EINKAUF_ANFRAGE',aktiv:true,standard:true,subjectTemplate:templateSubject}});}return route.fulfill({json:[{id:7,dokumentTyp:'EINKAUF_ANFRAGE',aktiv:true,standard:true,subjectTemplate:templateSubject,name:'Musteranfrage',htmlBody:'<p>Guten Tag</p>'}]});});
 await page.route('**/api/einkauf/anfragen/51/lieferanten/4/vorschau**', route=>route.fulfill({json:{revisionsNummer:2,eigeneKundennummer:'00017',antwortfrist:'2026-10-01',liefertermin:'2026-10-14',anlagen:[{id:8,dateiId:18,bedarfId:70,revision:'B',dateiname:'Profil.pdf',freigegeben:true}],version:0,vorschauHash:'hash-pa-51',subject:'Anfrage PA-00051',htmlBody:'<p>Guten Tag</p>',empfaenger:'einkauf@example.test',pdfDateiId:1,anlageVersionIds:[8]}}));
 await page.goto('/email-textvorlagen');
 await page.getByRole('button',{name:/bearbeiten/i}).first().click();
 await page.locator('#email-template-subject').fill('Alttext {{ANFRAGENUMMER}}');
 await page.getByRole('button',{name:/speichern/i}).click();
 await expect.poll(()=>templateSubject).toBe('Alttext {{ANFRAGENUMMER}}');
 ...
 await page.getByRole('combobox',{name:'Anfragefassung'}).click(); await page.getByText('Fassung 1').click(); await expect(page.getByText(/nur Lesen/)).toBeVisible(); await page.getByRole('combobox',{name:'Anfragefassung'}).click(); await page.getByText('Fassung 2').click(); await expect(page.getByRole('button',{name:/vorschau und freigabe/i})).toBeVisible();
 ...
 await expect(page.getByRole('heading',{name:/Versand prüfen und freigeben · Fassung 2/})).toBeVisible();
 await expect(page.getByText('Profil.pdf · Revision B · Freigegeben')).toBeVisible();
 await expect(page.getByText('00017',{exact:true})).toBeVisible();
 await designPruefung(page,testInfo,'anfrage-sendefreigabe',{primaerAktion:page.getByRole('button',{name:/freigeben und senden/i})});
