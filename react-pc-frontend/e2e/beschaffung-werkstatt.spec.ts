...
test('EN1090-Bedarf je Projekt und Vorrat: Werkstattprüfung bleibt erhalten, nur Fehlmenge geht in Bestellung', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: /Bedarf von Balkonanlage Musterstraße öffnen/ }).click();
  const vorhanden = page.getByRole('textbox', { name: 'Vorhanden Dichtband für Balkonanlage' });
  ...
  await expect(page.getByText('7,5 Meter', { exact: true })).toBeVisible();
  const downloadPromise = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Liste drucken' }).click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe('Bedarfsliste-19.pdf');
  const downloadPath = await download.path();
  expect(await readFile(downloadPath!, 'utf8')).toBe(pdf);
  expect(prints).toEqual([{ bedarfIds: [1] }]);
  await designPruefung(page, testInfo, 'beschaffung-werkstatt-geprueft', { primaerAktion: page.getByRole('button', { name: /In Bestellung übernehmen/ }) });
  await page.getByRole('button', { name: /In Bestellung übernehmen/ }).click();
  ...
  await page.getByRole('navigation', { name: 'Einkauf', exact: true }).getByRole('link', { name: 'Bedarf', exact: true }).click();
  await page.getByRole('button', { name: /Bedarf von Balkonanlage Musterstraße öffnen/ }).click();
  await expect(page.getByText('Im Bestellentwurf: 7,5 Meter')).toBeVisible();
  await expect(page.getByRole('button', { name: /In Bestellung übernehmen/ })).toBeDisabled();
  await page.reload();
  await expect(page.getByRole('button', { name: /In Bestellung übernehmen/ })).toBeDisabled();
  expect(orders).toHaveLength(1);
  ...
  await expect(page.getByRole('heading', { name: 'Für Werkstatt / auf Vorrat' })).toBeVisible();
  ...
  await page.getByLabel('Produktname Position 1').fill('Dichtmasse für Werkstatt');
  await page.getByLabel('Menge Position 1', { exact: true }).fill('3,5');
  await page.getByRole('combobox', { name: 'Einheit Position 1' }).click();
  await page.getByRole('option', { name: 'm (Meter)', exact: true }).click();
  await designPruefung(page, testInfo, 'beschaffung-vorrat-erfassen', { primaerAktion: page.getByRole('button', { name: 'Bedarf speichern', exact: true }) });
  await page.getByRole('button', { name: 'Bedarf speichern', exact: true }).click();
  ...
  await page.reload();
  await expect(page.getByText('Dichtmasse für Werkstatt', { exact: true })).toBeVisible();
  await expect(page.getByRole('row').filter({ hasText: 'Dichtmasse für Werkstatt' }).getByRole('cell', { name: '3,5 Meter', exact: true })).toHaveCount(2);
