...
import {
    AlertTriangle, CheckCircle2, Loader2, Lock, LockOpen, Scale, ShieldCheck,
} from 'lucide-react';
import { Button } from '../ui/button';
import { Card } from '../ui/card';
import { DatePicker } from '../ui/datepicker';
import { DecimalInput } from '../ui/decimal-input';
import { Dialog as SystemDialog, DialogHeader, DialogTitle } from '../ui/dialog';
...
function MonatsabschlussDialog({ jahr, monat, onClose, onFertig }: {
    ...
    const setFehler = useCallback((value: { text: string; hinweis?: string } | null) => {
        if (value) toast.error([value.text, value.hinweis].filter(Boolean).join(' '));
    }, [toast]);
...
function KassensturzDialog({ onClose, onFertig }: { onClose: () => void; onFertig: () => void }) {
    ...
    const [anzahlen, setAnzahlen] = useState<Record<number, string>>(() => Object.fromEntries(STUECKELUNG.map(s => [s.wert, '0'])));
    const [bemerkung, setBemerkung] = useState('');
    ...
    const zahlPruefung = validateNumberDrafts(anzahlen, Object.fromEntries(STUECKELUNG.map(s => [s.wert, { label: `Anzahl ${s.label}`, required: true, integer: true, min: 0 }])));
    const gezaehlt = zahlPruefung.valid ? STUECKELUNG.reduce((summe, s) => summe + s.wert * (zahlPruefung.values[s.wert] ?? 0), 0) : null;
    const gezaehltGerundet = gezaehlt === null ? null : Math.round(gezaehlt * 100) / 100;
    const differenz = erwartet === null || gezaehltGerundet === null ? null : Math.round((gezaehltGerundet - erwartet) * 100) / 100;
    const hatDifferenz = differenz !== null && differenz !== 0;
    const etwasGezaehlt = gezaehlt !== null && gezaehlt > 0;

    const speichere = async () => {
        if (!zahlPruefung.valid) { setFehler({ text: zahlPruefung.message }); return; }
        if (!etwasGezaehlt) { setFehler({ text: 'Bitte mindestens einen Schein oder eine Münze zählen.' }); return; }
        if (!stichtag) { setFehler({ text: 'Bitte einen Stichtag wählen.' }); return; }
        if (hatDifferenz && !bemerkung.trim()) { setFehler({ text: 'Bitte den Unterschied zur Kasse begründen.' }); return; }
        setSpeichern(true);
        setFehler(null);
        try {
            const stueckelung: Record<string, number> = {};
            for (const s of STUECKELUNG) {
                const anzahl = zahlPruefung.values[s.wert] ?? 0;
    ...
    return (
        <Dialog titel="Kasse zählen" onClose={onClose} breit>
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
                {STUECKELUNG.map(s => (
                    <label key={s.wert} className="block">
                        ...
                        <DecimalInput min={0} integer required
                            value={anzahlen[s.wert] ?? ''}
                            onChange={value => setAnzahlen(a => ({ ...a, [s.wert]: value }))}
                            placeholder="0"
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm rounded-lg bg-slate-50 p-3">
                ...
                <dd className="text-right font-semibold text-slate-900 tabular-nums">{gezaehltGerundet == null ? '–' : `${euro(gezaehltGerundet)} €`}</dd>
...
/** Gemeinsamer Dialog übernimmt Fokus und Portal-Auswahl. */
function Dialog({ titel, onClose, children, breit }: {
    titel: string; onClose: () => void; children: React.ReactNode; breit?: boolean;
}) {
    return <SystemDialog open onOpenChange={open => { if (!open) onClose(); }} aria-label={titel} className={`w-full ${breit ? 'max-w-3xl' : 'max-w-xl'}`}>
        <DialogHeader><DialogTitle>{titel}</DialogTitle></DialogHeader>
        <div className="mt-4 overflow-y-auto min-h-0">{children}</div>
    </SystemDialog>;
}
