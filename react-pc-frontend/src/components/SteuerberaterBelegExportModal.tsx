...
const buildAnredeZeile = (
  ...
  switch (anrede) {
    case "HERR":
      return safeNachname
        ? `Sehr geehrter Herr ${safeNachname},`
        : "Sehr geehrter Herr,";
    case "FRAU":
      return safeNachname
        ? `Sehr geehrte Frau ${safeNachname},`
        : "Sehr geehrte Frau,";
    case "FAMILIE":
      return safeNachname
        ? `Sehr geehrte Familie ${safeNachname},`
...
export function SteuerberaterBelegExportModal({
  ...
  const [steuerberaterListe, setSteuerberaterListe] = useState<
    SteuerberaterKontakt[]
  >([]);
  const [selectedSteuerberaterId, setSelectedSteuerberaterId] = useState<
    number | null
  >(null);
  const [selectedAnsprechpartnerId, setSelectedAnsprechpartnerId] = useState<
    number | null
  >(null);
  ...
  const selectedSteuerberater = useMemo(
    () =>
      steuerberaterListe.find((sb) => sb.id === selectedSteuerberaterId) ||
      null,
  ...
  const selectedAnsprechpartner = useMemo(
    () =>
      selectedSteuerberater?.ansprechpartnerListe.find(
        (ap) => ap.id === selectedAnsprechpartnerId,
      ) || null,
  ...
  const verfuegbareEmails = useMemo<string[]>(() => {
    ...
    (selectedSteuerberater.weitereEmails || []).forEach((e) => {
      if (e) set.add(e);
    });
    (selectedSteuerberater.ansprechpartnerListe || []).forEach((ap) => {
      if (ap.email) set.add(ap.email);
    });
    ...
    (sig: string, anredeZeile: string) => {
      ...
      const rows = entries
        .map((e) => {
          ...
          const hinweis =
            hinweisParts.length > 0 ? hinweisParts.join("<br>") : "";
          ...
          return `<tr style="${rowStyle}">${cells
            .map(
              (c) =>
      ...
      const teilweiseCount = entries.filter(
        (e) => e.aufteilungsModus === "TEILWEISE",
      ).length;
...
`;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([liste, firma, sig]) => {
        if (cancelled) return;
        const sbListe: SteuerberaterKontakt[] = Array.isArray(liste)
          ? liste
          : [];
        const firmenname =
          firma && typeof firma === "object" && firma.firmenname
  ...
  useEffect(() => {
    ...
    const anredeZeile = selectedAnsprechpartner
      ? buildAnredeZeile(
          selectedAnsprechpartner.anrede,
          selectedAnsprechpartner.nachname,
        )
  ...
  const ansprechpartnerOptions = (
    selectedSteuerberater?.ansprechpartnerListe || []
  ).map((ap) => {
    const fullName = [ap.vorname, ap.nachname].filter(Boolean).join(" ");
    return { value: String(ap.id), label: fullName || "(ohne Namen)" };
  });
  const empfaengerOptions = verfuegbareEmails.map((e) => ({
    value: e,
    label: e,
  }));
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-5xl max-h-[95vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-rose-50 rounded-t-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-900">
                Belegaufstellung Kasse an Steuerberater
              </h2>
              <p className="text-sm text-slate-500">
                {anhang
                  ? `ZIP-Anhang: ${anhang.dateiname}`
                  : loadingEntries
                    ? "Belege werden geladen…"
                    : `${entries.length} validierte Kassen-Belege im Monat`}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={handleSend}
              disabled={sending || loadingEntries}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="bg-rose-50/60 border border-rose-100 rounded-lg p-3 text-xs text-slate-600 flex items-start gap-2">
            ...
            <span>
              {anhang
                ? "Die Kassenunterlagen werden als ZIP-Datei angehängt."
                : "Diese E-Mail enthält die Aufstellung als HTML-Tabelle direkt im Body – keine PDF-Anhänge. Die physischen Belege liegen dem Steuerberater bereits vor; die Beleg-Nr. dient als Referenz."}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 max-w-md">
            <div className="space-y-1">
              ...
              <Select
                ...
                options={MONATSNAMEN.map((label, i) => ({
                  value: String(i + 1),
                  label,
                }))}
          ...
          {loadingSteuerberater ? (
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Loader2 className="w-4 h-4 animate-spin" /> Steuerberater werden
              geladen...
            </div>
          ) : steuerberaterListe.length === 0 ? (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-lg text-rose-700 text-sm">
              Es ist noch kein Steuerberater hinterlegt. Bitte unter{" "}
              <strong>Firma → Steuerberater</strong>
              ...
              {steuerberaterListe.length > 1 && (
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    value={
                      selectedSteuerberaterId !== null
                        ? String(selectedSteuerberaterId)
                        : ""
                    }
                    onChange={(v) =>
                      setSelectedSteuerberaterId(v ? Number(v) : null)
                    }
                  ...
                  <Select
                    ...
                    value={
                      selectedAnsprechpartnerId !== null
                        ? String(selectedAnsprechpartnerId)
                        : ""
                    }
                    onChange={(v) =>
                      setSelectedAnsprechpartnerId(v ? Number(v) : null)
                    }
                    placeholder="Ansprechpartner wählen"
                  />
                ) : (
                  <p className="text-xs text-slate-600">
                    Kein Ansprechpartner hinterlegt – Anrede wird „Sehr geehrte
                    Damen und Herren" verwendet.
                  </p>
                )}
              </div>
            </div>
