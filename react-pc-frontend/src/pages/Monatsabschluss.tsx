...
export default function Monatsabschluss() {
    ...
    const [auswahl, setAuswahl] = useState<Stand[]>([]);
    ...
    function waehle(stand: Stand) { generation.current++; setAuswahl(prev => prev.some(s => key(s) === key(stand)) ? prev.filter(s => key(s) !== key(stand)) : [...prev, stand]); }
    ...
    return <div className="p-6 space-y-6">
        ...
        {laedt ? <p role="status" className="bg-slate-100 rounded-lg p-8 motion-safe:animate-pulse">Monatsdaten werden geladen …</p> : daten && <section className="bg-white border border-slate-200 rounded-lg shadow-sm p-4 space-y-4" aria-label="Monatsübersicht">
            ...
            <div className="flex justify-between items-center text-sm"><p>{daten.totalElements} Mitarbeiter · Seite {filter.page + 1} von {Math.max(1, Math.ceil(daten.totalElements / 50))}</p><div className="flex gap-2"><Button variant="outline" size="sm" aria-label="Vorherige Seite" disabled={filter.page === 0 || busy} onClick={() => setFilter(f => ({ ...f, page: f.page - 1 }))}><ChevronLeft className="w-4 h-4" />Zurück</Button><Button variant="outline" size="sm" aria-label="Nächste Seite" disabled={(filter.page + 1) * 50 >= daten.totalElements || busy} onClick={() => setFilter(f => ({ ...f, page: f.page + 1 }))}>Weiter<ChevronRight className="w-4 h-4" /></Button></div></div>
        </section>}
