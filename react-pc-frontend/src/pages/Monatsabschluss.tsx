...
export default function Monatsabschluss() {
    ...
    async function abschliessen() {
        ...
        try {
            const ok = await confirm({ title: 'Monat jetzt abschließen?', message: `${monate[filter.monat - 1]} ${filter.jahr}: ${staende.length} Mitarbeiter abschließen? Bitte vorher alle Zeiten prüfen. Der geprüfte Stand bleibt festgehalten.`, confirmLabel: 'Abschließen', variant: 'warning' });
    ...
    // "Sonstige" nur zeigen, wenn es dort überhaupt Stunden gibt
    const mitSonstigen = !!daten && (daten.summen.sonstigeAbwesenheitStunden !== 0 || daten.items.some(z => z.kennzahlen.sonstigeAbwesenheitStunden !== 0));
    const spalten = felder.filter(([id]) => mitSonstigen || id !== 'sonstigeAbwesenheitStunden');
    ...
    return <div className="p-6 space-y-6">
        <header className="flex flex-col md:flex-row justify-between gap-4 md:items-end mb-8">
            <div>
                <p className="text-sm font-semibold text-rose-600 uppercase tracking-wide">Zeiterfassung</p>
                <h1 className="text-3xl font-bold text-slate-900">MONATSABSCHLUSS</h1>
                <p className="text-slate-500 mt-1">Zeiten prüfen und den Monatsstand für Ihre Mitarbeiter festhalten.</p>
            </div>
        </header>
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
                <p className="font-medium text-slate-700">{auswahl.length} ausgewählt</p>
                {recht && <div title={grund}><Button size="sm" className="bg-rose-600 hover:bg-rose-700 text-white" disabled={!!grund || !daten} onClick={abschliessen}><FileCheck aria-hidden="true" className="w-4 h-4 mr-2" />{busy ? 'Abschluss läuft …' : 'Monat jetzt abschließen'}</Button></div>}
            </div>
            <Button variant="outline" size="sm" disabled={laedt || busy} onClick={() => { generation.current++; setAuswahl([]); setRevision(v => v + 1); }}><RefreshCw aria-hidden="true" className="w-4 h-4 mr-2" />Aktualisieren</Button>
        </div>
        ...
        {laedt ? <p role="status" className="bg-slate-100 rounded-lg p-8 motion-safe:animate-pulse">Monatsdaten werden geladen …</p> : daten && <section className="bg-white border border-slate-200 rounded-lg shadow-sm p-4 space-y-4" aria-label="Monatsübersicht">
            ...
            <p className="text-sm text-slate-600">Alle Angaben in Stunden. Gesamt = {spalten.filter(([id]) => bestandteile.has(id)).map(([, name]) => name.replace('\u00AD', '')).join(' + ')}. Korrektur: von Hand im Büro nachgetragene Stunden.{mitSonstigen && ' Sonstige sind Fortbildungen und ältere Abwesenheiten, die beim Abschluss noch nicht aufgeteilt wurden.'}</p>
            <table className="w-full table-fixed text-sm"><thead><tr className="border-b text-left text-slate-600"><th className="w-9 py-3"><input type="checkbox" className="accent-rose-600 h-4 w-4" aria-label="Alle gefilterten Mitarbeiter auswählen" checked={all} disabled={!daten.auswahl.length || busy} onChange={() => { generation.current++; setAuswahl(all ? [] : daten.auswahl); }} /></th><th className="w-[13%]">Mitarbeiter</th>{spalten.map(([id, name]) => <th key={id} className="text-right px-1 break-words">{name}</th>)}<th className="w-[10%] pl-3">Stand</th><th className="w-28 text-right pr-2">Details</th></tr></thead>
            <tbody>{daten.items.map(zeile => <tr key={key(zeile.referenz)} className="border-b border-slate-100 hover:bg-slate-50"><td className="py-3"><input type="checkbox" className="accent-rose-600 h-4 w-4" aria-label={`${zeile.mitarbeiterName} auswählen`} disabled={busy} checked={auswahl.some(s => key(s) === key(zeile.referenz))} onChange={() => waehle({ ...zeile.referenz, version: zeile.version, festgeschrieben: zeile.festgeschrieben })} /></td><th scope="row" className="text-left font-medium break-words pr-2">{zeile.mitarbeiterName}</th>{spalten.map(([id]) => <td key={id} className="text-right tabular-nums px-1">{zahl(zeile.kennzahlen[id])}</td>)}<td className="pl-3"><span className={`inline-block rounded px-2 py-1 text-xs ${zeile.festgeschrieben ? 'bg-slate-100 text-slate-800' : 'bg-amber-50 text-amber-800'}`}>{zeile.festgeschrieben ? 'Abgeschlossen' : 'Noch offen'}</span>{zeile.festgeschriebenAm && <p className="text-xs text-slate-500 mt-1">{datum(zeile.festgeschriebenAm)}</p>}</td><td className="pr-2"><div className="flex items-center justify-end gap-1 whitespace-nowrap"><Button variant="ghost" size="sm" className="px-2" title="Verlauf anzeigen" aria-label={`Verlauf für ${zeile.mitarbeiterName}`} onClick={() => setVerlaufZeile(zeile)}><History className="w-4 h-4" /></Button><Link className="p-2 rounded hover:bg-rose-50 text-rose-700 focus:ring-2 focus:ring-rose-500" title="Im Kalender prüfen" aria-label={`Kalender für ${zeile.mitarbeiterName}`} to={`/zeitbuchungen?${new URLSearchParams({ mitarbeiterId: String(zeile.referenz.mitarbeiterId), jahr: String(zeile.referenz.jahr), monat: String(zeile.referenz.monat) })}`}><Calendar className="w-4 h-4" /></Link>{zeile.festgeschrieben && recht && <Button variant="ghost" size="sm" className="px-2 text-rose-700 hover:bg-rose-50" title="Monat wieder öffnen" aria-label={`Monat für ${zeile.mitarbeiterName} wieder öffnen`} onClick={() => oeffneMonat(zeile)}><LockOpen className="w-4 h-4" /></Button>}</div></td></tr>)}</tbody>
            <tfoot><tr className="font-semibold bg-slate-50"><th colSpan={2} className="text-left py-3 pr-2">Alle gefilterten Mitarbeiter</th>{spalten.map(([id]) => <td key={id} className="text-right tabular-nums px-1">{zahl(daten.summen[id])}</td>)}<td colSpan={2} /></tr></tfoot></table>
