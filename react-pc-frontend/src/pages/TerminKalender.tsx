            ...
            <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm mb-6">
                ...
                <div className="relative">
                    ...
                    <button
                        ...
                        className="flex items-center justify-between w-44 px-3 py-2 bg-white border border-slate-300 rounded-md hover:bg-slate-50 transition-colors"
                    >
                ...
                <div>
                    ...
                    <div className="flex bg-slate-100 rounded-md p-0.5">
                        <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded transition-all flex items-center gap-1 ${ansicht === 'monat' ? 'bg-white text-rose-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded transition-all flex items-center gap-1 ${ansicht === 'woche' ? 'bg-white text-rose-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded transition-all flex items-center gap-1 ${ansicht === 'tag' ? 'bg-white text-rose-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                                }`}
                        >
                    ...
                    <div className="divide-y divide-slate-200">
                        {wochenTage.map(tag => {
                            ...
                            return (
                                <div
                                    key={tag.datum}
                                    className={`flex gap-4 p-4 transition-colors cursor-pointer hover:bg-rose-50/30 ${isWeekend ? 'bg-slate-50/50' : ''
                                ...
                                >
                                                    ...
                                                    >
                                                        ...
                                                        {(eintrag.projektName || eintrag.kundeName || eintrag.lieferantName) && (
                                                            <div className="flex flex-wrap gap-2 mt-2 text-xs text-slate-500">
                                                                {eintrag.projektName && <span>📁 {eintrag.projektName}</span>}
                                                                {eintrag.kundeName && <span>👤 {eintrag.kundeName}</span>}
                                                                {eintrag.lieferantName && <span>🚚 {eintrag.lieferantName}</span>}
                                                            </div>
                                                        )}
                                                    </div>
                                                );
                                    ...
                                    <button
                                        ...
                                        className="p-2 h-fit text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                                    >
...
function EventModal({ datum, eintrag, mitarbeiter, currentUserMitarbeiterId, onClose, onSave }: EventModalProps) {
    ...
    const searchAngebote = async (term: string) => {
        try {
            const res = await fetch(`/api/angebote?q=${encodeURIComponent(term)}`);
            if (!res.ok) return [];
            const data = await res.json();
                            ...
                            <div className="flex flex-wrap gap-2">
                                {FARB_OPTIONEN.map(f => (
                                    <button
                                        ...
                                        className={`w-8 h-8 rounded-full border-2 transition-all ${farbe === f.value
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                        <SearchableSelect
                                            ...
                                            onSearch={searchAngebote}
                                            selectedLabel={eintrag?.angebotBetreff || undefined}
