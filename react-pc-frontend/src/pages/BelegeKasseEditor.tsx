...
import {
  ...
  type SteuerberaterPaketAnhang,
} from "../components/kasse/SteuerberaterPaketDialog";
...
export default function BelegeKasseEditor() {
  ...
  const [loadError, setLoadError] = useState<"forbidden" | "network" | null>(
    null,
  );
  ...
  const monatsanfangIso = isoDatum(
  ...
  const [steuerberaterPaketAnhang, setSteuerberaterPaketAnhang] =
    useState<SteuerberaterPaketAnhang | null>(null);
  ...
  useEffect(() => {
    const belegId = Number(
      new URLSearchParams(window.location.search).get("belegId"),
    );
  ...
  useEffect(() => {
    const hatOffene = belege.some(
      (b) => b.kiAnalyseStatus === "PENDING" || b.kiAnalyseStatus === "LAEUFT",
    );
    ...
    switch (activeTab) {
      ...
      case "kasse":
        return belege.filter(
          (b) =>
            (b.belegKategorie === "KASSE_EINNAHME" ||
              b.belegKategorie === "KASSE_AUSGABE") &&
            b.status !== "VERWORFEN" &&
            match(b),
        );
      default:
  ...
  return (
      ...
      actions={
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={loadBelege} disabled={loading}>
            <RefreshCw
              className={loading ? "w-4 h-4 mr-2 animate-spin" : "w-4 h-4 mr-2"}
            />
            Aktualisieren
          </Button>
          {activeTab === "kasse" && (
            <Button
              variant="outline"
              onClick={() => setMonatsExportOpen(true)}
          ...
          <Button
            variant="outline"
            onClick={() => setSteuerberaterPaketOpen(true)}
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-slate-200">
        <TabButton
          active={activeTab === "eingang"}
          onClick={() => setActiveTab("eingang")}
          icon={<FileQuestion className="w-4 h-4" />}
          label="Eingang (Validierung)"
          badge={eingangsCount > 0 ? eingangsCount : undefined}
        />
        <TabButton
        ...
        />
        <TabButton
          active={activeTab === "kasse"}
          onClick={() => setActiveTab("kasse")}
          icon={<Coins className="w-4 h-4" />}
          label="Kassenbuch"
        />
        <TabButton
          active={activeTab === "auswertung"}
      ...
      {activeTab === "kasse" ? (
        ...
        <Card className="p-12 text-center border-amber-200 bg-amber-50">
          ...
          <p className="font-medium text-amber-900">
            Keine Berechtigung für Belege
          </p>
          <p className="text-sm mt-2 text-amber-800 max-w-md mx-auto">
            Dein Account hat keine Sicht-Berechtigung für Belege (Typ{" "}
            <code className="font-mono">BELEG</code>). Lass dich unter{" "}
            <strong>Administration → Lieferanten-Dokumentenrechte</strong> für
            die Abteilung Buchhaltung freischalten — oder prüfe, ob dein
            Frontend-Login mit einem Mitarbeiter-Datensatz (gleiche E-Mail)
            verknüpft ist.
          </p>
        </Card>
      ) : loadError === "network" ? (
        <Card className="p-12 text-center border-red-200 bg-red-50">
          ...
          <p className="text-sm mt-2 text-red-800">
            Netzwerk- oder Serverfehler. Prüfe die Browser-Konsole (F12) für
            Details.
          </p>
        </Card>
      ) : gefiltert.length === 0 ? (
        <Card className="p-12 text-center text-slate-500">
          ...
          <p className="font-medium">
            {activeTab === "eingang"
              ? "Keine offenen Belege zur Validierung"
              : "Keine Belege gefunden"}
          </p>
          <p className="text-sm mt-1">
            Belege werden über die Handy-App gescannt oder hier hochgeladen.
          </p>
        </Card>
      ) : (
      ...
      <SteuerberaterBelegExportModal
        ...
        onClose={() => {
          setSteuerberaterEmailOpen(false);
          setSteuerberaterPaketAnhang(null);
        }}
      ...
      <SteuerberaterPaketDialog
        ...
        onEmailPaket={(anhang) => {
          setSteuerberaterPaketAnhang(anhang);
          setSteuerberaterEmailOpen(true);
        }}
      />
    </PageLayout>
  );
...
function MonatsExportModal({ onClose }: { onClose: () => void }) {
  ...
  const jahre: number[] = [];
  for (let j = heute.getFullYear() + 1; j >= heute.getFullYear() - 5; j--)
    jahre.push(j);
      ...
      >
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="monats-export-titel" className="font-bold text-slate-900">
                Kassenbuch-Export (Steuerberater)
              </h2>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-rose-50/60 border border-rose-100 rounded-lg p-3 text-xs text-slate-600 flex items-start gap-2">
            ...
            <span>
              Das PDF zeigt das Kassen-Konto als T-Konto (Eingang | Ausgang) mit
              Anfangs- und Endsaldo. Übergib es zusammen mit dem Ordner der
              Belegfotos an den Steuerberater.
            </span>
          </div>
...
function BelegRow({ beleg, onClick }: { beleg: Beleg; onClick: () => void }) {
  ...
  const teilweise =
    beleg.aufteilungsModus === "TEILWEISE" && beleg.betragFirmaBrutto != null;
  const anzeigeBrutto = teilweise
    ? beleg.betragFirmaBrutto
    : beleg.betragBrutto;
  return (
    <button
      onClick={onClick}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <span className="font-semibold text-slate-900 truncate">
            {beleg.belegNummer ||
              beleg.lieferantName ||
              beleg.kiVorgeschlagenerLieferant ||
              beleg.originalDateiname ||
              `Beleg #${beleg.id}`}
          </span>
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${KATEGORIE_FARBE[beleg.belegKategorie]}`}
          >
            {KATEGORIE_LABELS[beleg.belegKategorie]}
          </span>
          {teilweise && (
            <span
              className="text-xs px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-200 inline-flex items-center gap-1"
          ...
          {beleg.eingangsrechnungId != null && (
            <span
              className="text-xs px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 inline-flex items-center gap-1"
          ...
          {!beleg.istUmbuchung && (
            <span className={`text-xs px-2 py-0.5 rounded-full ${ki.cls}`}>
              {ki.label}
            </span>
          )}
        </div>
        <div className="text-sm text-slate-500 flex items-center gap-4 flex-wrap">
          ...
          {beleg.sachkontoBezeichnung && (
            <span className="inline-flex items-center gap-1 text-slate-600">
              ...
              {beleg.sachkontoNummer ? `${beleg.sachkontoNummer} ` : ""}
              {beleg.sachkontoBezeichnung}
            </span>
          )}
          {beleg.uploadedByName && (
            <span>Hochgeladen von {beleg.uploadedByName}</span>
          )}
        </div>
      </div>
      <div className="text-right flex-shrink-0">
        <div className="font-semibold text-slate-900">
          {formatEuro(anzeigeBrutto)} €
        </div>
        {teilweise ? (
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <Card className="p-12 text-center text-slate-500">
          ...
          <p>Noch keine Auswertung geladen.</p>
        </Card>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiTile
              label="Erträge"
              value={`${formatEuro(auswertung.summeErtrag)} €`}
              icon={<Banknote className="w-5 h-5 text-emerald-600" />}
            />
            <KpiTile
              label="Aufwand"
              value={`${formatEuro(auswertung.summeAufwand)} €`}
              icon={<CreditCard className="w-5 h-5 text-amber-600" />}
            />
            <KpiTile
              label="Privatentnahmen"
              value={`${formatEuro(auswertung.summePrivat)} €`}
              icon={<Wallet className="w-5 h-5 text-fuchsia-600" />}
            />
            <KpiTile
              label="Ergebnis"
              value={`${formatEuro(ergebnis)} €`}
              icon={
                <BarChart3
                  className={`w-5 h-5 ${ergebnis < 0 ? "text-red-500" : "text-emerald-600"}`}
                />
              }
              highlight
            />
          ...
          {auswertung.summeOhneKonto > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800 inline-flex items-center gap-2">
              ...
              {formatEuro(auswertung.summeOhneKonto)} € sind noch keinem
              Sachkonto zugeordnet.
            </div>
          )}
                ...
                {auswertung.zeilen.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-8 text-slate-400">
                      Keine validierten Belege im Zeitraum.
                    </td>
                  </tr>
                ) : (
                  auswertung.zeilen.map((z: AuswertungZeile, i) => (
                    <tr
                      key={z.sachkontoId ?? `none-${i}`}
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        {z.kontoTyp ? (
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${typFarbe[z.kontoTyp]}`}
                          >
                            {typLabel[z.kontoTyp]}
                          </span>
                        ) : (
