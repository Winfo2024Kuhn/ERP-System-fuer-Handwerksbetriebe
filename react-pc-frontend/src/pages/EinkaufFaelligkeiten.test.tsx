...
it('zeigt nur offene Fälligkeiten und bereitet eine Nachfrage vor ohne sie zu senden', async () => {
  ...
  global.fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/api/auth/me') return { ok: true, json: async () => ({ id: 9, roles: ['ADMIN'] }) } as Response;
    if (path.includes('/api/einkauf/faelligkeiten?')) return { ok: true, json: async () => ({ content: [
    ...
    if (path.endsWith('/nachfrage')) return { ok: true, json: async () => ({ typ: 'ANFRAGE_ANTWORTFRIST', vorgangId: 81, beteiligungId: 83, vorlageId: 3, vorlageVersion: 2, empfaenger: 'lieferant@example.test', subject: 'Rückmeldung zu PA-81', htmlBody: '<p>Bitte antworten.</p>', fehlendeNachweise: [] }) } as Response;
  ...
  await waitFor(() => expect(paths.some(path => path.includes('/nachfrage'))).toBe(true));
  expect(paths.some(path => path.includes('/senden') || path.endsWith('/antworten'))).toBe(false);
});
