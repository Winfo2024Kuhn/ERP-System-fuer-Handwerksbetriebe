...
describe('AnfrageLieferanten', () => {
 beforeEach(() => { global.fetch = vi.fn(async (input: RequestInfo | URL) => { const url=String(input); if(url.includes('/versandstatus')) return {ok:true,json:async()=>[]} as Response; if(url==='/api/email-textvorlagen') return {ok:true,json:async()=>[{id:7,dokumentTyp:'EINKAUF_ANFRAGE',aktiv:true,standard:true}]} as Response; if(url.includes('/vorschau?')) return {ok:true,json:async()=>({version:2,vorschauHash:'hash-abc',subject:'Anfrage PA-51',htmlBody:'<p>Guten Tag</p>',empfaenger:'einkauf@example.test',pdfDateiId:2,anlageVersionIds:[4]})} as Response; return {ok:true,json:async()=>({status:'ERFOLGREICH'})} as Response; }); });
 it('zeigt individuelle Empfänger und bindet Freigabe an den geladenen Vorschauhash', async () => {
   const user=userEvent.setup(); render(<ToastProvider><AnfrageLieferanten anfrageId={51} revisionId={2} lieferanten={[{id:4,lieferantenname:'Musterlieferant',status:'AUSSTEHEND',version:0}]} /></ToastProvider>);
