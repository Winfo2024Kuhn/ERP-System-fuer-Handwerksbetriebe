...
it('zeigt die konkrete Revisionsvorschau und verlangt deren Bestätigung vor der Freigabe', async () => {
  ...
  const onClose = vi.fn();
  render(<ToastProvider><BestellfreigabeDialog bestellungId={73} onClose={onClose} onReleased={onReleased} /></ToastProvider>);
  ...
  await user.click(screen.getByRole('button', { name: 'PDF ansehen' }));
  await user.click(screen.getByTitle('In neuem Tab öffnen'));
  expect(open).toHaveBeenCalledWith('/api/einkauf/pdf-vorschau/55', '_blank', 'noopener,noreferrer');
