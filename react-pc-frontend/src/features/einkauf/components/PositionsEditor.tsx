...
import type { Dokumentart, Einheit } from '../types';
...
export function PositionsEditor({ value, onChange, readOnly = false }: PositionsEditorProps) {
  ...
  const anlageHinzufuegen = () => {
    if (!/^\d+$/.test(anlageId) || Number(anlageId) <= 0) return;
  ...
  return <section aria-label="Position bearbeiten" className="space-y-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    {value.art === 'ARTIKEL' && value.artikelId !== null && <p className="text-sm text-slate-600">Artikelstamm-ID {value.artikelId} · Preis wird bei der Bestellung ausdrücklich bestätigt.</p>}
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div><Label htmlFor="position-referenz">Interne Artikelnummer</Label><Input id="position-referenz" value={value.interneReferenz} onChange={e => setText('interneReferenz', e.target.value)} readOnly={readOnly} /></div>
    ...
    <section aria-labelledby="anlagen-title" className="space-y-3">
      <div><h3 id="anlagen-title" className="font-semibold text-slate-800">Zeichnungen und Anlagen</h3><p className="text-sm text-slate-500">Es zählen konkrete Anlagenversionen; eine ID allein ist keine Freigabe.</p></div>
      <div className="flex flex-wrap gap-2">{value.anlageVersionIds.map(id => <span key={id} className="rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm">Anlagenversion {id}{!readOnly && <button type="button" className="ml-2 text-slate-500 hover:text-rose-700" aria-label={`Anlagenversion ${id} entfernen`} onClick={() => set('anlageVersionIds', value.anlageVersionIds.filter(existing => existing !== id))}><X aria-hidden="true" className="inline h-3.5 w-3.5" /></button>}</span>)}</div>
      {!readOnly && <div className="flex max-w-lg items-end gap-2"><div className="flex-1"><Label htmlFor="anlage-id">Anlagenversion-ID</Label><Input id="anlage-id" inputMode="numeric" value={anlageId} onChange={e => setAnlageId(e.target.value)} /></div><Button type="button" variant="outline" size="sm" onClick={anlageHinzufuegen}><Plus aria-hidden="true" className="h-4 w-4" />Version hinzufügen</Button></div>}
    </section>
