...
export function PositionsEditor({ value, onChange, readOnly = false, anlagen = [] }: PositionsEditorProps) {
  ...
  return <section aria-label="Position bearbeiten" className="space-y-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="max-w-sm"><Label htmlFor="positionsart">Positionsart</Label><Select id="positionsart" aria-label="Positionsart" value={value.art} options={[{ value: 'ARTIKEL', label: 'Artikel' }, { value: 'FREITEXT', label: 'Freies Material' }, { value: 'ZEICHNUNGSTEIL', label: 'Zeichnungsteil' }]} onChange={art => onChange({ ...value, art: art as PositionDraft['art'], artikelId: null })} disabled={readOnly} /></div>
    {value.art === 'ARTIKEL' && value.artikelId !== null && <p className="text-sm text-slate-600">Artikel aus dem Stamm ausgewählt · Der Preis kann bei der Bestellung ergänzt werden.</p>}
