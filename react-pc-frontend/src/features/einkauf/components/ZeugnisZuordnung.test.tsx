...
it('lädt gespeicherte Zuordnungen und prüft mit der aktuellen Chargenversion nach Neuaufruf', async () => {
  const original = fetchMock.getMockImplementation()!;
  let geprueft = false;
  fetchMock.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
    if (String(input) === '/api/einkauf/berechtigungen') return new Response(JSON.stringify(['LESEN', 'ZEUGNIS_PRUEFEN']));
    if (String(input) === '/api/einkauf/zeugnisse?bestellungId=73') return new Response(JSON.stringify([{ ...expectations[0], zuordnungen: [{ zuordnungId: 1001, erwartungId: 111, chargeId: 101, status: geprueft ? 'GEPRUEFT' : 'ZUGEORDNET', version: geprueft ? 8 : 7, materialFreigegeben: geprueft }] }]));
    if (String(input) === '/api/einkauf/zeugnisse/1001/pruefen') { geprueft = true; return new Response(JSON.stringify({ materialFreigegeben: true })); }
    return original(input, init);
  });
  render(<ToastProvider><ZeugnisZuordnung bestellungId={73} /></ToastProvider>);
  await userEvent.click(await screen.findByRole('button', { name: 'Zeugnis prüfen · Charge 101' }));
  await userEvent.type(screen.getByLabelText('Begründung'), 'Charge geprüft');
  await userEvent.click(screen.getByRole('button', { name: 'Prüfung speichern' }));
  await screen.findByText('Charge 101: Material freigegeben');
  const sent = fetchMock.mock.calls.find(call => call[0] === '/api/einkauf/zeugnisse/1001/pruefen');
  expect(JSON.parse(sent![1].body as string)).toMatchObject({ version: 7, grundlageVersion: 'V3' });
});
