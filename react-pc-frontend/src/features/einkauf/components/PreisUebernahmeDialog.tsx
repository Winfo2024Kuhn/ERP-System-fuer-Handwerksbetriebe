...
export function PreisUebernahmeDialog(props: Props) {
  const toast = useToast(); const [scope, setScope] = useState('STANDARD'); const [projekt, setProjekt] = useState(''); const [abMenge, setAbMenge] = useState(''); const [bisMenge, setBisMenge] = useState(''); const [grund, setGrund] = useState(''); const [busy, setBusy] = useState(false);
  const speichern = async () => {
    if (!grund.trim()) { toast.error('Bitte bestätigen Sie kurz, warum dieser Preis übernommen wird.'); return; }
    const projectId = scope === 'PROJEKT' ? validateDecimalInput(projekt, { label: 'Projekt-ID', required: true, min: 1, integer: true }) : null;
    if (projectId && !projectId.valid) { toast.error(projectId.message); return; }
