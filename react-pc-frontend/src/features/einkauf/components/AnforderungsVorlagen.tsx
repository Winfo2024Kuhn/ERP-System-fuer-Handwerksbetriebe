...
export function AnforderungsVorlagen() {
  ...
  const artikelId = artikel ? String(artikel.id) : '';
  const projektId = projekt ? String(projekt.id) : '';
  ...
  const ladeVorgaben = async () => {
    ...
    if ((!positiveId(article) && !positiveId(project)) || (article && !positiveId(article)) || (project && !positiveId(project))) {
      const message = 'Bitte wählen Sie einen Artikel oder ein Projekt aus.'; setFehler(message); toast.error(message); return;
    }
  ...
  return <section aria-label="Zeugnisvorgaben pflegen" className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <StammdatenAuswahl art="Artikel" value={artikel} onChange={wahl => { setArtikel(wahl); setVorgaben([]); setGespeichert(null); }} />
      <StammdatenAuswahl art="Projekt" value={projekt} onChange={wahl => { setProjekt(wahl); setVorgaben([]); setGespeichert(null); }} />
    </div>
