  ...
  return <nav aria-label="Einkauf" className="rounded-lg border border-slate-200 bg-white p-2 shadow-sm">
    <ul className="flex flex-wrap gap-1">
      {schritte.map(({ id, text, href, icon: Icon }) => <li key={id}>
        <NavLink to={href} aria-current={active === id ? 'page' : undefined}
          className={({ isActive }) => `inline-flex min-h-10 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-500 ${active === id || isActive ? 'bg-rose-50 text-rose-800' : 'text-slate-700 hover:bg-slate-50'}`}>
