...
export function BelegAuswahl({ bestellungId, typ, value, onChange }: BelegAuswahlProps) {
  ...
  return <section aria-label="Beleg auswählen" className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
    ...
    <div className="flex flex-wrap items-center gap-2">
      <Button type="button" variant="outline" size="sm" disabled={wirdGespeichert} onClick={() => uploadInput.current?.click()}>PDF hochladen</Button>
      <span className="text-xs text-slate-600">PDF-Datei vom Gerät auswählen</span>
      <input ref={uploadInput} aria-label="PDF hochladen" type="file" accept="application/pdf,.pdf" disabled={wirdGespeichert} tabIndex={-1}
        onChange={(event) => { void ladeHoch(event.currentTarget.files?.[0]); event.currentTarget.value = ''; }} className="sr-only" />
    </div>
  </section>;
}
