...
export function VersandStatus({ versand, onKlaeren }: { versand: VersandDto; onKlaeren?: () => void }) {
  if (versand.status === 'ANGENOMMEN') return <div role="status" className="flex items-start gap-3 rounded-lg border border-slate-300 bg-white p-4 text-slate-900">
  ...
  if (versand.status === 'UNKLAR') return <div role="alert" className="flex items-start gap-3 rounded-lg border border-rose-300 bg-rose-50 p-4 text-rose-950">
    ...
    {onKlaeren && <button type="button" onClick={onKlaeren} className="shrink-0 rounded-md border border-rose-300 bg-white px-3 py-1.5 text-sm font-medium text-rose-900 hover:bg-rose-100 focus:outline-none focus:ring-2 focus:ring-rose-500">Versand klären</button>}
  </div>;
