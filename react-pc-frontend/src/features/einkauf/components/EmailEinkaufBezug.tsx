...
export function EmailEinkaufBezug({ email }: Props) {
    ...
    return <div data-testid="einkauf-mail-bezug" className="border-b border-rose-100 bg-rose-50 px-4 py-2 text-sm text-slate-700">
        ...
        {!email.zuordnungPruefen && email.einkaufTyp === 'ANFRAGE' && email.einkaufVorgangId
            ? <a className="font-medium text-rose-700 underline underline-offset-2" href={`/einkaufsanfragen/${email.einkaufVorgangId}`}>{state}</a>
            : state}
