...
export function BestellfreigabeDialog({ bestellungId, onClose, onReleased }: { bestellungId: number; onClose: () => void; onReleased: (versand: VersandDto) => void }) {
  ...
  const pdfUrl = vorschau?.pdfDateiId ? toSafeResourceUrl(`/api/einkauf/anlagen/${vorschau.pdfDateiId}`) : null;
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }} className="w-[min(46rem,calc(100vw-2rem))]">
    ...
    {pdfOpen && pdfUrl && <DocumentPreviewModal doc={{ url: pdfUrl, title: vorschau?.subject ?? 'Bestellung.pdf' }} onClose={() => setPdfOpen(false)} />}
  </Dialog>;
}
