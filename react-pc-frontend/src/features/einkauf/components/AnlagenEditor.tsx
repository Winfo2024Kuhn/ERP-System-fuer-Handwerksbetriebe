...
export function AnlagenEditor({ anlagen, ausgewaehlt, onChange, zeichnungsteil = false }: {
  anlagen: readonly EinkaufAnlage[]; ausgewaehlt: readonly number[]; onChange: (ids: number[]) => void; zeichnungsteil?: boolean;
}) {
  const freigegebeneZeichnung = anlagen.some(anlage => anlage.freigegeben && ausgewaehlt.includes(anlage.id));
  return <fieldset className="space-y-2 rounded-lg border border-slate-200 p-3">
    ...
    {anlagen.length === 0 ? <p className="text-sm text-slate-600">Für diesen Bedarf sind noch keine Anlagen vorhanden.</p> : <div className="space-y-2">{anlagen.map(anlage => <label key={anlage.id} className="flex min-w-0 items-start gap-2 rounded-md bg-slate-50 p-2 text-sm">
      <input type="checkbox" aria-label={`${anlage.dateiname} auswählen`} checked={ausgewaehlt.includes(anlage.id)} onChange={event => onChange(event.target.checked ? [...ausgewaehlt, anlage.id] : ausgewaehlt.filter(id => id !== anlage.id))} />
