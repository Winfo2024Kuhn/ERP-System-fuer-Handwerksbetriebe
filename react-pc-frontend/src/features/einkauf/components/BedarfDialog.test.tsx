...
vi.mock('./StammdatenAuswahl', () => ({ StammdatenAuswahl: ({ art, value: wert, onChange: geändert }: { art: string; value: { id: number; name: string } | null; onChange: (wert: { id: number; name: string } | null) => void }) => <button type="button" onClick={() => geändert(wert ? null : { id: 4, name: 'Projekt Nord · Halle' })}>{art}: {wert?.name ?? 'auswählen'}</button> }));
...
it('meldet ungültige Eingaben mit Feldfehler und Toast vor dem Speichern', async () => {
  ...
  render(<ToastProvider><BedarfDialog offen schließen={vi.fn()} gespeichert={vi.fn()} ausgangsbedarf={bedarf} /></ToastProvider>);
...
it('erhält beim Bearbeiten Dokumente, Schnittdaten, Bearbeitung, Oberfläche, Beschaffungsangaben und Lieferdaten', async () => {
  const bedarf: BedarfResponse = { id: 44, version: 3, position: { art: 'ARTIKEL', artikelId: 9, interneReferenz: 'MAT-9', zeichnungsnummer: 'Z-9', zeichnungsrevision: 'C', bezeichnung: 'Stahlträger', werkstoff: 'S355', abmessung: 'IPE 200', basis: { menge: 12, einheit: 'METER', stueckzahl: 3, einzelLaengeMm: 4000, kgJeMeter: 22, faktorQuelle: 'Datenblatt' }, schnittForm: 'GEHRUNG', winkelLinks: '30', winkelRechts: '45', bearbeitung: 'Entgraten', oberflaeche: 'Verzinkt', dokumente: [{ art: 'ZEUGNIS_3_1', grundlage: 'EN 10204', grundlageVersion: '2025', fachlichBestaetigt: true }], anlageVersionIds: [81], beschaffungsdetails: { lieferantId: 8, kategorieId: 3, schnittbildId: 5, schnittAchseId: 2, externeArtikelnummer: 'MS-4711' } }, liefergruppe: { lieferadresse: 'Hof Nord', bedarfstermin: '2026-10-02', projektId: 4, lagerzweck: null }, mengen: { bedarf: 12, lagergedeckt: 0, angefragt: 0, reserviert: 0, bestellt: 0, geliefert: 0, storniert: 0, ungedeckt: 12, disponierbar: 12 }, nachpflegeErforderlich: false, historischerHinweis: null };
