...
export function ZeugnisZuordnung({ bestellungId }: { bestellungId: number }) {
  ...
  const ladeDaten = useCallback(async () => {
    ...
    try {
      ...
      setZuordnungen(expected.flatMap(item => item.zuordnungen ?? []));
  ...
  return <section aria-label="Zeugnisse und Materialfreigabe" className="space-y-4">
    ...
    {laden ? <p role="status">Zeugnisse werden geladen …</p> : erwartungen.length === 0 ? <p className="rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-600">Für diese Bestellung sind keine Zeugnisse hinterlegt.</p> : <>
      ...
      {pruefCharge && darfPruefen && <Dialog open onOpenChange={open => { if (!open) setPruefCharge(null); }} className="w-[min(36rem,calc(100vw-2rem))]">
        <DialogHeader className="px-6 pt-6"><DialogTitle>Zeugnis prüfen · Charge {pruefCharge.chargeId}</DialogTitle><DialogDescription>Das Prüfergebnis entscheidet über die Materialfreigabe dieser Charge.</DialogDescription></DialogHeader><DialogContent className="space-y-3 px-6 py-4">
        ...
        </DialogContent><DialogFooter className="px-6 pb-6"><Button type="button" onClick={() => void pruefen()} disabled={speichert}>Prüfung speichern</Button><Button type="button" variant="outline" onClick={() => setPruefCharge(null)} disabled={speichert}>Abbrechen</Button></DialogFooter>
      </Dialog>}
    </>}
  </section>;
}
