...
describe('QuellenLink', () => {
  it('opens a safe document URL and strips unsafe markup from source text', () => {
    const quelle = { typ: 'DOKUMENT', id: 48, bezeichnung: '<img src=x onerror=alert(1)>Lieferschein', betrag: null, url: '/api/einkauf/pdf-vorschau/48', html: '<p>Geprüft</p><script>alert(1)</script>' };
    render(<QuellenLink quelle={quelle} />);
  ...
  it('renders unsafe resource URLs as inert text', () => {
    render(<QuellenLink quelle={{ typ: 'DOKUMENT', id: 49, bezeichnung: 'Unsichere Quelle', betrag: null, url: 'javascript:alert(1)' }} />);
