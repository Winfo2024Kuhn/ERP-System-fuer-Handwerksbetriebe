...
it('lädt manuell gepflegte Vorgaben zur Artikel-ID mit Grundlage und Version', async () => {
  const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
    ...
    if (String(input).startsWith('/api/artikel?')) return new Response(JSON.stringify({ artikel: [{ id: 15, artikelnummer: 'MAT-15', produktname: 'Stahlprofil' }], gesamt: 1 }));
    if (String(input).startsWith('/api/artikel?')) return new Response(JSON.stringify({ artikel: [{ id: 15, artikelnummer: 'MAT-15', produktname: 'Stahlprofil' }], gesamt: 1 }));
    return new Response('[]', { status: 200 });
  });
  global.fetch = fetchMock as typeof fetch;
  render(<MemoryRouter><ToastProvider><AnforderungsVorlagen /></ToastProvider></MemoryRouter>);

  await userEvent.click(screen.getByRole('button', { name: 'Artikel auswählen' }));
  await userEvent.click(await screen.findByRole('button', { name: 'Stahlprofil auswählen' }));
